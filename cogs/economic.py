...
class Economic(commands.Cog):
    ...
    @commands.command(aliases=["remove-money"], brief="Забирает деньги у пользователя", usage="remove-money [cash | bank] <@member> <amount>")
    @commands.has_any_role(902849136041295883, 933769903910060153, 902841113734447214)
    async def remove_money(self, ctx, amount: int, member: discord.Member, destination: Optional[str] = 'cash'):
        if destination.lower() not in ["cash", "bank"]:
            embed = discord.Embed(
